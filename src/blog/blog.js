export default class Blog {
    ...
    initialize() {
        this.showBlog.addEventListener('click', () => {
            this.insertPosts();
        });

        this.postContainerUl.addEventListener('click', (event) => {
            if(event.target.className === 'like') {
                this.likeList.add(event.target.previousElementSibling);
                event.target.className = 'unlike';
                event.target.innerText = '찜취소';
            }
            else if(event.target.className === 'unlike') {
                this.likeList.delete(event.target.previousElementSibling);
